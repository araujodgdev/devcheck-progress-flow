...
export default function ChecklistDetails() {
  ...
  const togglePublicAccess = async () => {
    try {
      ...
      if (error) throw error;
      setChecklist(data);
      toast.success(data.is_public ? 'Checklist tornada pública' : 'Checklist tornada privada');
    } catch (err: any) {
  ...
  const updateChecklistItemDueDate = async (item: ChecklistItem, date: Date | undefined) => {
    try {
      const { data, error } = await supabase
        .from('checklist_items')
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    Esta ação não pode ser desfeita. Isso excluirá permanentemente a checklist
              ...
              <div className="flex items-center">
                <Popover>
                  <PopoverTrigger asChild>
                    <Button variant={'ghost'} className={cn('h-8 w-8 p-0 font-normal', !item.due_date && 'text-muted-foreground')}>
                      {item.due_date ? (
